    ...
    if(PARSE_COMPONENTS)
        ...
        if(ROCM_PACKAGE_COMPONENT_DEPENDENCIES)
            foreach(COMP_DEP IN LISTS ROCM_PACKAGE_COMPONENT_DEPENDENCIES)
                ...
                rocm_package_add_rpm_dependencies(COMPONENT "${_downstream}" DEPENDS "${CPACK_RPM_${_upstream_uc}_PACKAGE_NAME} >= ${CPACK_PACKAGE_VERSION}")
                rocm_package_add_deb_dependencies(COMPONENT "${_downstream}" DEPENDS "${CPACK_DEBIAN_${_upstream_uc}_PACKAGE_NAME} >= ${CPACK_PACKAGE_VERSION}")
            endforeach()
        endif()
    endif()
    include(CPack)
...
macro(rocm_set_comp_cpackvar HEADER_ONLY components)
    ...
    foreach(COMPONENT ${components})
        ...
        if(CPACK_DEBIAN_${COMPONENT_UC}_PACKAGE_NAME STREQUAL "")
            if(CPACK_DEBIAN_PACKAGE_NAME STREQUAL "")
                set(CPACK_DEBIAN_PACKAGE_NAME "${CPACK_PACKAGE_NAME}")
            endif()
            string(TOLOWER "${CPACK_DEBIAN_PACKAGE_NAME}-${COMPONENT}" CPACK_DEBIAN_${COMPONENT_UC}_PACKAGE_NAME)
        else()
            string(REGEX REPLACE "<PACKAGE_NAME>" "${CPACK_PACKAGE_NAME}" CPACK_DEBIAN_${COMPONENT_UC}_PACKAGE_NAME "${CPACK_DEBIAN_${COMPONENT_UC}_PACKAGE_NAME}")
        endif()
        if(CPACK_RPM_${COMPONENT_UC}_PACKAGE_NAME STREQUAL "")
            if(CPACK_RPM_PACKAGE_NAME STREQUAL "")
                set(CPACK_RPM_PACKAGE_NAME "${CPACK_PACKAGE_NAME}")
            endif()
            string(TOLOWER "${CPACK_RPM_PACKAGE_NAME}-${COMPONENT}" CPACK_DEBIAN_${COMPONENT_UC}_PACKAGE_NAME)
        else()
            string(REGEX REPLACE "<PACKAGE_NAME>" "${CPACK_PACKAGE_NAME}" CPACK_RPM_${COMPONENT_UC}_PACKAGE_NAME "${CPACK_RPM_${COMPONENT_UC}_PACKAGE_NAME}")
        endif()
    endforeach()
endmacro()
...
macro(rocm_package_setup_component COMPONENT_NAME)
    ...
    if(DEFINED PARSE_DEPENDS)
        cmake_parse_arguments(DEPENDS "" "" "COMMON;DEBIAN;RPM;COMPONENT" ${PARSE_DEPENDS})
