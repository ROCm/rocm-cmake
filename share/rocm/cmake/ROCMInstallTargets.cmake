...
function(rocm_parse_multi_list LIST PREFIX)
    set(id)
    set(elements)
    set(labels)
    foreach(ELEMENT ${${LIST}})
        if(ELEMENT IN_LIST ARGN)
            if(id)
                string(REPLACE ";" " " elements "${elements}")
                list(APPEND _${id} "${elements}")
                set(elements)
            endif()
            set(id ${ELEMENT})
            list(APPEND labels ${ELEMENT})
    ...
    foreach(label ${labels})
        set(${PREFIX}_${label} "${_${label}}" PARENT_SCOPE)
    endforeach()
endfunction()

function(rocm_write_package_deps CONFIG_TEMPLATE)
    set(DEPENDS ${ARGN})
    rocm_parse_multi_list(DEPENDS DEP PACKAGE PKG_CONFIG)
    foreach(DEPEND ${DEP_PACKAGE})
        rocm_write_package_template_function(${CONFIG_TEMPLATE} find_dependency "${DEPEND}")
    endforeach()
    foreach(DEPEND ${DEP_PKG_CONFIG})
        rocm_write_package_template_function(${CONFIG_TEMPLATE} find_pkg_config_dependency "${DEPEND}")
    endforeach()
endfunction()
