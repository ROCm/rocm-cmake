    ...
    foreach(SOURCE ${SOURCES})
        if(NOT "${SOURCE}" MATCHES "(h|hpp|hxx)$")
            ...
            if(CLANG_TIDY_CACHE)
                ...
                string(REPLACE ";" " " CLANG_TIDY_COMMAND_STR "${CLANG_TIDY_COMMAND}")
                file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/${tidy_target}.cmake "
                    execute_process(COMMAND ${CMAKE_COMMAND} --build ${CMAKE_CURRENT_BINARY_DIR} --target ${BASE_SOURCE}.i OUTPUT_VARIABLE PP_OUT RESULT_VARIABLE RESULT)
                    if(NOT RESULT EQUAL 0)
                        message(FATAL_ERROR \"Could not preprocess ${BASE_SOURCE}.i\")
                    endif()
                    string(REPLACE \"Preprocessing CXX source to \" \"\" PP_FILE \"\${PP_OUT}\")
                    string(STRIP \"\${PP_FILE}\" PP_FILE)
                    file(MD5 \${PP_FILE} PP_HASH)
                    execute_process(COMMAND ${CLANG_TIDY_COMMAND_STR} ${SOURCE} --dump-config OUTPUT_VARIABLE TIDY_CONFIG)
                    ...
                    if(EXISTS \${HASH_FILE})
                        ...
                        if(\${CACHED_HASH} STREQUAL \${HASH})
                            set(RUN_TIDY Off)
                        endif()
                    endif()
                    if(RUN_TIDY)
                        execute_process(
                            COMMAND ${CLANG_TIDY_COMMAND_STR} ${SOURCE} \"-export-fixes=${CLANG_TIDY_FIXIT_DIR}/${TARGET}-${tidy_file}.yaml\"
